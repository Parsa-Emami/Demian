import { characterFrameWorldSize } from '../characters/CharacterVisualContract.js';
...
export function spriteDrawMetrics(camera, entity, frame) {
    ...
    const bodyWidth = Math.max(22, worldMetrics.bodyWidth * camera.pixelsPerUnit);
    const bodyHeight = Math.max(22, worldMetrics.bodyHeight * camera.pixelsPerUnit);
    const scaleX = (worldMetrics.frameWidth * camera.pixelsPerUnit / Math.max(1, frame.w))
...
export function drawSpriteCharacter(ctx, camera, entity, {
    label = '',
    player = entity?.isPlayerControlled,
    opacity = 1,
    eliminated = false,
} = {}) {
    ...
    ctx.globalAlpha = Math.min(1, Math.max(0.08, Number(opacity) || 0));
