...
from character_art_registry import character_name, load_profile, resolve_targets, speech_payload
...
def build_atlas(character: str, variant: str, cell: int, names_by_sequence: dict[str, list[str]]) -> dict[str, object]:
    ...
    return {
        ...
        "companion": profile.get("companion") if isinstance(profile.get("companion"), dict) else None,
        "pivot": {"x": 0.5, "y": 0.96},
...
def main() -> None:
    characters = resolve_targets(
        sys.argv[1:],
        ("{character}-spritesheet-v4.png", "{character}-atlas.json"),
    )
    if not characters:
        raise SystemExit("No V4 character packs were found for V5 generation.")

    for character in characters:
        build_character(character)

    # Audit exactly the packs generated by this invocation. This keeps the builder
    # reusable for newly added characters without requiring every legacy source pack.
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "tools" / "audit_character_sprite_packs.py"),
            "--version",
            "5",
            "--write-metadata",
            "--strict",
