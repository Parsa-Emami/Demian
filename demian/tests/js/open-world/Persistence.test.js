...
test('Open World save store persists checksummed state and rejects tampering', () => {
    ...
    assert.deepEqual(store.load({ worldId: manifest.id, maxWorldVersion: manifest.version }).position, { x: 4, z: 4 });
...
test('save point activation unlocks fast travel and restores discovery', () => {
    ...
    const system = new SavePointSystem({
        ...
        stateProvider: () => ({ worldId: manifest.id, worldVersion: manifest.version, position: { x: 14.8, z: 5.6 } }),
    });
    system.activate('save-counter');
    assert.equal(discovery.isSavePointUnlocked('save-counter'), true);
    ...
    assert.equal(restoredDiscovery.isSavePointUnlocked('save-counter'), true);
});
